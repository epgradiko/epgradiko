...
function optimize_table( $table ){
	global $dbh, $settings;
	reclog( 'optimizeTable::'.$settings->tbl_prefix.$table.'最適化開始', EPGREC_DEBUG );
	if( mysqli_query( $dbh, 'ALTER TABLE '.$settings->tbl_prefix.$table.' ENGINE InnoDB' )) {
		reclog( 'optimizeTable::'.$settings->tbl_prefix.$table.'最適化終了', EPGREC_DEBUG );
	}else{
		reclog( 'OptimizeTable::'.$settings->tbl_prefix.$table.'最適化失敗('.mysqli_error($dbh).')', EPGREC_ERROR );
	}
}
run_user_regulate();
