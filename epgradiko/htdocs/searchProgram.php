	...
	if( $mode != 'timeshift' ){
		...
		if( isset( $settings->mirakc_timeshift ) && $settings->mirakc_timeshift != 'none' ){
			...
			foreach($channels_raw as $channel_raw){
				...
				$ch = new DBRecord( CHANNEL_TBL, 'channel_disc', $channel_disc );
				$ch_id = $chid_list[] = (int)$ch->id;
				array_push( $stations, ["id" => $ch_id,
					...
					foreach( $regions->station as $station) {
						if( $station->timefree == 1 ){
							...
							if( count($db_programs) ){
								$ch = new DBRecord( CHANNEL_TBL, 'channel_disc', 'EX_'.$station->id );
								$ch_id = $chid_list[] = (int)$ch->id;
								array_push( $stations, ["id" => $ch_id,
											"name" => $station->name,
											"type" => 'EX',
...
if(isset( $_POST['do_search'] )) {
	...
	if( isset($_GET['keyword_id']) ) {
		...
		if( DBRecord::countRecords( KEYWORD_TBL, 'WHERE id='.$keyword_id ) == 0 ){
			echo '<html><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"></head><body onLoad="var ref = document.referrer;var key = \'keywordTable.php\';if( ref.indexOf(key) > -1 ){location.href = key;}else{if( ref.indexOf(\'searchProgram.php\') > -1 ){location.href = key;}else{location.href = ref;}}"></body></html>';
			exit( 1 );
		}
...
try{
	$programs = array();
	if( $do_keyword ){
		$precs = Keyword::search( $mode, $search, $use_regexp, $collate_ci, $ena_title, $ena_desc, $search_marks,
...
EXIT_REV:;
					...
					foreach( $rev as $cnt => $r ){
						...
						if( $r->autorec ){
							$arr['genres'] = '<a href="searchProgram.php?keyword_id='.$r->autorec.'" title="キーワード編集にジャンプ">自動</a><br>';
						} else {
							$arr['genres'] = '手動<br>';
						}
						if( $r->program_id ){
	...
	$types = array();
	$type_names = '';
	if( array_search( 'GR', array_column( $stations, 'type' ) ) !== FALSE ){
		$arr = array();
		$arr['name'] = '地デジ';
		$arr['value'] = 'GR';
		if( $typeGR ){
	...
	if( array_search( 'BS', array_column( $stations, 'type' ) ) !== FALSE ){
		$arr = array();
		$arr['name'] = 'BS';
		$arr['value'] = 'BS';
		if( $typeBS ){
	...
	if( array_search( 'CS', array_column( $stations, 'type' ) ) !== FALSE ){
		$arr = array();
		$arr['name'] = 'CS';
		$arr['value'] = 'CS';
		if( $typeCS ){
	...
	if( array_search( 'EX', array_column( $stations, 'type' ) ) !== FALSE ){
		$arr = array();
		$arr['name'] = 'ラジオ';
		$arr['value'] = 'EX';
		if( $typeEX ){
...
//			foreach( $autorec_modes as $loop => $auto_mode ){
//				if( isset($auto_mode['tsuffix']) ) {
//					$trans_set .= '<option value="'.$loop.'"'.( (int)$trans_unit['mode']===$loop ? ' selected ':'' ).'>'.$auto_mode['name'].'</option>';
//				}
//			}
//			$trans_set .= '</select> '
	...
	if( $keyword_id ) $smarty->assign( 'sitetitle', 'キーワード編集' );
	else if( $mode == 'timeshift' ) $smarty->assign( 'sitetitle', 'タイムシフト番組検索' );
	else $smarty->assign( 'sitetitle', '予約番組検索' );
	...
	$smarty->display('searchProgram.html');
}
catch( exception $e ) {
	exit( $e->getMessage() );
}
